...
- **A shader can declare the element sizes it actually has.** `groupUniforms` derives cohort boundaries from a *size field*, which defaults to a dense uniform sample of 64 values — fine for a continuous field (a dot grid's radius sweeping down the canvas), and the reason natural clusters collapses to one cohort on every shader wired so far: a uniform sample has no gaps to split on. A shader with genuinely discrete elements registers its own instead:
  ...
  - **The same declaration feeds the GLSL.** `u_element[11]` is a `vec2` per element — `.x` its size 0–1, `.y` how much ink it carries relative to an average one — read with `paletteElement(id)` using the same id passed to `applyPaletteGroups`. One declaration, so the CPU's boundaries and the shader's lookup cannot disagree about how big anything is. `shader-base` seeds the table to `(0.5, 1)`, not zeros: an unset weight clamps to 0.05 and would make `pow(density, 20)` — everything unprinted.
- **scaling-letters is the first shader where natural clusters works.** Its word is drawn as up to eleven letters at author-set point sizes, so the field is genuinely discrete and has gaps to split on. Measured with five letters at 500/80/300/90/460pt: clusters finds **5 cohorts** with bounds at 0.012, 0.274, 0.714 and 0.952 — one per letter — where every continuous-field shader returns 1.
  - **Its element id is free and cannot split a letter.** `letterIdx` is already resolved in the GLSL for every letter-count layout, is constant across a letter's whole cell, and the size is a table lookup keyed off it — so the per-fragment-size failure the line and dot shaders each needed fixing for cannot occur here by construction.
  - **Weight is point size squared**, normalized to a mean of 1. Measured: a letter at 300pt carries ~21x the ink of the same letter at 60pt, against the 25x `size²` predicts. What it deliberately does not model is glyph shape — at one size a `W` carries ~3x the ink of an `I`, and a full stop about a sixteenth of an `M`. Measuring that means reading back the letter atlas, which would cost the size-field function its purity; the point-size term spans ~100x across the slider range and dominates.
  - **With the per-letter toggle off, every letter is the same size and one cohort is the honest answer** — the field is flat, and clusters correctly finds nothing to split.
  - Grouping is applied to `fillCol`, the letter's tile, before the outline is mixed in, so an outline stays its own color — which is black, which is the shirt.
