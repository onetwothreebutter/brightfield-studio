...
- When committing an AI-generated image asset, add its prompt to the provenance log in `docs/image-prompts.md`
...
## Dev commands
- Dev preview: `npm run dev` → `scripts/dev.mjs`, which runs `shopify theme dev --store brightfield-2.myshopify.com` on a free port (9292 preferred)
- Manual push: `npm run push` → `shopify theme push --store brightfield-2.myshopify.com --theme 143500345459`
...
- Analytics: `snippets/analytics.liquid` (rendered from `layout/theme.liquid`) registers each provider — GA4, PostHog — as a sink behind its own theme setting and `request.design_mode == false`, and syncs Shopify's Customer Privacy consent to all of them once. Fire events with `window.bfTrack(name, params)`; never call `gtag`/`posthog` from a section. The logic `<script data-bf-analytics>` carries no Liquid so `test/analytics.test.js` can execute it verbatim. Session replay is behind `posthog_session_replay` and only starts after the same opt-in (inputs masked, canvas recorded at low fps). The browser funnel ends at `begin_checkout`, so purchases are stitched server-side: `assets/cart.js` writes `window.bfAnalyticsId()` (the PostHog distinct_id, null unless opted in) into the `_posthog_distinct_id` cart attribute on the Checkout submit, and the worker's orders/paid webhook captures `purchase` against it with a uuid derived from the order id so redeliveries dedupe. Needs `POSTHOG_PROJECT_KEY` on the worker; Shop Pay / dynamic checkout buttons skip the cart form and so carry no id.
...
- `FINISH_CONTROLS` / `FINISH_CONTROLS_PRE` / `FINISH_CONTROLS_POST` — the shared Finish-section control arrays (see "Finish section" below)
...
The Finish section is always the last section in the controls array, and it is built from shared globals defined in `shader-controls-base.liquid` — never retype the entries:
- `FINISH_CONTROLS_PRE` — the `Finish` header, `u_opacity`, `u_grain_mode` (5 grain types: Organic / Blue Noise / Scratches / Crosshatch / Half-tone), the per-mode distress pairs `u_distress_0..4` + `u_distress_scale_0..4` (each row `grainDependent` on its mode, with per-mode labels, ranges and defaults), the Half-tone extras (`u_halftone_angle`, `u_halftone_luma`, `u_halftone_shape`), and `u_distress_falloff`
...
- **Size groups reach the shaders that expose an element size.** A fragment shader has no shape list, but it already computed how big the element under each fragment is, and it can name which element that is. So the CPU sends only what it alone can decide — the cohort boundaries, the color each one drew, the print odds and the seed — and the GLSL does the lookup and the roll. No readback, no ID buffer, no second pass. A shader opts in with one line:
  ...
  **Sparks roll per element in the GLSL too.** The engine keeps sparks out of the cohort draw on purpose (a spark that wins a tier stops being a find) and relies on `sparkOverride` per shape — which a shader never ran, so with grouping on a shader could not show a spark at *any* weight: measured **0/40 designs on rise-shirt at weight 60**. `groupUniforms` now sends `u_spark_color`, `u_spark_chance` (the sparks' combined share, capped by `maxShare`) and `u_spark_seed` (its own `element-spark` stream, so a spark never moves a print/skip decision), and `paletteSparkOr(col, id)` rolls it with the same `paletteRoll32` and 24-bit quantization. `applyPaletteGroups` applies it for the joint-form shaders; a post-processing shader uses the three-argument `paletteGroupedColor(col, size, id)` so the spark goes through the same post-processing as the cohort colour — rise-shirt does. Measured after: 12/12 seeds at preview resolution.
  - **One colour uniform.** With several sparks the heaviest (by actual share — `maxShare` caps only the chance) lands and the others add to the odds. The policy is the engine's `assigner.sparks()`, not adapter arithmetic, so it cannot drift from the CPU path; it also skips a spark whose colour would crash the renderer and expands `#abc` shorthand. Size conditions are dropped as for the slot draw; region and afterColor cannot apply — a fragment has no position or neighbour — which is a real divergence from the ungrouped slot draw, where the assigner honours both.
  - **Chance is per element, so how often a spark shows depends on the element count.** 2% of ~1500 dots is a certainty; 2% of 20 lines is one design in three. The lab's round swatch in the slot strip reports the *visible* rate — chance × print density, since the print roll can still drop a sparked element — and the spark joins the units the distribution table counts.
  - `mapPalette` never emits these unless grouping is on; the product page restores control keys only, so this reaches the product no more than the cohorts do.
