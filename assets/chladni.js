(function () {
  ...
  var fragSrc = [
    ...
    '}',
    '',
    '// Evaluates the Chladni pattern color + alpha at any uv (post pos/scale',
    '// transform). baseColorOut receives the gradient color for text fill.',
    'vec4 designEval(vec2 uv, out vec3 baseColorOut) {',
    ...
    '  vec3 baseColor;',
    '  vec4 px = designEval(uv, baseColor);',
    '  vec3 col = px.rgb;',
    ...
    '    // supersample) so dots shrink toward pattern edges instead of slicing.',
    ...
    '        vec4 smp   = designEval(sUV, bcDummy);',
    '        covSum += smp.a;',
    ...
    '    col = dotColor;',
    '  } else {',
    '    alpha = applyDistress(px.a, dUV, u_distress, u_distress_scale, u_grain_mode, u_distress_falloff, dot(px.rgb, vec3(0.299, 0.587, 0.114)), vigMask) * u_opacity;',
    '    col = col * vigMask;',
    '  }',
  ...
  window.ShaderBase.create({
    animateValues:  true,
    instantKeys:    ['u_opacity', 'u_distress_0', 'u_distress_scale_0', 'u_distress_1', 'u_distress_scale_1', 'u_distress_2', 'u_distress_scale_2', 'u_distress_3', 'u_distress_scale_3', 'u_grain_mode', 'u_halftone_shape', 'u_distress_falloff'],
    fragSrc: fragSrc,
