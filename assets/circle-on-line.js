(function () {
  ...
  var fragSrc = [
    ...
    '  // phase is fract(warped * u_line_count), so its floor is the line index —',
    '  // the element print density rolls against, one roll per stripe.',
    '  highp float lineId = floor(warped * u_line_count);',
    '  float invP     = 1.0 / max(u_power, 0.0001);',
    '  float cellLo   = pow(lineId / u_line_count, invP);',
    ...
    '  float lineWidthMid = mix(u_width_top, u_width_bot, lineT);',
    '  float lineSize = lineSpan < 0.0001 ? 0.5',
    '                 : (lineWidthMid - min(u_width_top, u_width_bot)) / lineSpan;',
    '  finalPal = applyPaletteGroups(finalPal, lineSize, lineId, lineWgt);',
    '',
    ...
    '  // lineSize is (lineWidth - min) / span and lineWidth is mix(top, bot, t),',
    '  // so size is just t — which makes the mapping back to the screen exact',
    '  // rather than approximate: t = 0 at the top of the circle, 1 at the bottom.',
    '  // When the bottom lines are the thinner ones the field runs the other way,',
    '  // so the two ends swap and the labels follow the cohorts rather than the',
    '  // geometry. Drawn after gamma so the glyphs stay crisp white, and after the',
    '  // grain so a heavy distress setting cannot eat them.',
    '  float lblSpan = abs(u_width_bot - u_width_top);',
    '  if (u_group_debug > 0.5 && lblSpan >= 0.0001) {',
    '    float lblTop  = 0.5 + u_radius;',
    '    float lblDrop = u_radius * 2.0;',
    '    bool  lblFlip = u_width_bot < u_width_top;',
    '    float lblY0   = lblFlip ? lblTop - lblDrop : lblTop;',
    '    float lblY1   = lblFlip ? lblTop : lblTop - lblDrop;',
    '    float lbl = paletteGroupLabelMask(uv, 0.045, lblY0, lblY1, 0.055,',
    '                                      u_resolution.y / max(u_resolution.x, 1.0));',
    '    encoded = mix(encoded, vec3(1.0), lbl);',
    '    alpha   = max(alpha, lbl);',
    '  }',
    '',
