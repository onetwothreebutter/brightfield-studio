(function () {
  ...
  var fragSrc = [
    ...
    '',
    '  float blurredR = max(s0.r, max(s1.r * w1, max(s2.r * w2, max(s3.r * w3,',
    '                   max(s4.r * w4, max(s5.r * w5, max(s6.r * w6, s7.r * w7)))))));',
    '  float blurredG = max(s0.g, max(s1.g * w1, max(s2.g * w2, max(s3.g * w3,',
    '                   max(s4.g * w4, max(s5.g * w5, max(s6.g * w6, s7.g * w7)))))));',
    '',
    '  float texR = mix(s0.r, blurredR, u_blur_enabled);',
    '  float texG = mix(s0.g, blurredG, u_blur_enabled);',
    '',
