(function () {
  ...
  var fragSrc = [
    ...
    '  vec3  blendedLch = mix(mix(seg01, seg12, step(1.0 / 3.0, palTFinal)), seg23, step(2.0 / 3.0, palTFinal));',
    ...
    '    // supersample) so dots shrink toward glyph edges instead of slicing.',
    ...
    '      }',
    '    }',
    '    vec3 dotColor  = covSum > 0.001 ? inkSum / covSum : color;',
    ...
    '    color = dotColor;',
    '  } else {',
    '    alpha = applyDistress(px.a, dUV, u_distress, u_distress_scale, u_grain_mode, u_distress_falloff, dot(color, vec3(0.299, 0.587, 0.114)), vigMask) * u_opacity;',
    '  }',
