(function () {
  ...
  var fragSrc = [
    ...
    '}',
    '',
    '// Evaluates the design color + alpha at any uv (post offset_y).',
    ...
    '  float aaLine   = max(fwidth(lineSDF) * 0.5, aaFixed);',
    ...
    '  float aaCirc     = max(fwidth(circSDF) * 0.5, aaFixed);',
    ...
    '    // supersample) so dots shrink toward line/circle/text edges instead',
    '    // of slicing.',
    ...
    '    finalColor = dotColor;',
    '  } else {',
    '    alpha = px.a * applyDistress(1.0, dUV, u_distress, u_distress_scale, u_grain_mode, u_distress_falloff, dot(finalColor, vec3(0.299, 0.587, 0.114)), vigMask) * u_opacity;',
    '  }',
    '',
