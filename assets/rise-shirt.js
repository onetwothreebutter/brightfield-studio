(function () {
  ...
  var fragSrc = [
    ...
    '// Alpha is pre-multiplied by the canvas-bounds mask so halftone coverage',
    '// sampling sees the design end at the artwork edge.',
    ...
    '',
    '  // Size grouping belongs at the call site, not inside paletteAt: radius is',
    '  // mix(effectiveMax, u_min_radius, mixFactor), so this dot sits at size',
    '  // 1 - mixFactor. The text grid below shares paletteAt but not that',
    '  // meaning — every text dot is u_text_radius, one single size.',
    '  // The dot is the element: its halftone cell is both its identity and its',
    '  // size. Both are read at the cell, never at this fragment — a per-fragment',
    '  // size lets a cohort boundary fall inside one dot and paint its top and',
    '  // bottom halves different colours.',
    '  //',
    '  // highp on every operand, not just the result: GLSL evaluates an',
    '  // expression at the precision of its *operands*, so `highp float id =',
    '  // <mediump maths>` converts a value that has already lost precision. The',
    '  // product reaches rows*cols, and a mediump float stops counting in ones',
    '  // past 2048 — adjacent dots would start sharing a print roll on the phones',
    '  // this is mostly viewed on.',
    '  highp float dotRow = floor(gridUv.y);',
    '  highp float dotColumn = floor(gridUv.x);',
    '  highp float dotId  = dotRow * u_cols + dotColumn;',
    '  float cellY        = (dotRow + 0.5) / max(u_rows, 1.0);',
    '  float cellMix      = mix(cellY, 1.0 - cellY, u_invert);',
    ...
    '  return vec4(finalCol, alpha * inDesign);',
    '}',
    '',
    'void main() {',
    ...
    '      }',
    '    }',
    '    vec3 dotColor  = covSum > 0.001 ? inkSum / covSum : finalCol;',
    ...
    '    finalCol = dotColor;',
    '  } else {',
    '    alpha = applyDistress(px.a, dUV, u_distress, u_distress_scale, u_grain_mode, u_distress_falloff, dot(finalCol, vec3(0.299, 0.587, 0.114)), vigMask) * u_opacity;',
