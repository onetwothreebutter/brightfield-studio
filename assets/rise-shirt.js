(function () {
  ...
  var fragSrc = [
    ...
    '  // The dot is the element: its cell in the halftone grid is its identity,',
    '  // so print density rolls once per dot rather than once per cohort.',
    '  highp float dotId = floor(gridUv.y) * u_cols + floor(gridUv.x);',
    '  vec3 dotRaw      = applyPaletteGroups(paletteAt(mixFactor), 1.0 - mixFactor, dotId);',
