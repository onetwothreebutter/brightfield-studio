(function () {
  ...
  var fragSrc = [
    ...
    '',
    '// Evaluates the design color + alpha at any uvCoord (post pos/scale',
    '// transform). Corner rounding uses derivative-based cell aspect, which',
    '// falls back to 1.0 when sampled at constant halftone-cell positions.',
    'vec4 designEval(vec2 uvCoord) {',
    ...
    '    letterIdx = isRight * mix(2.0, 1.0, isTop);',
    '    vec2 luv1 = vec2(gridUV.x * 1.5, gridUV.y);',
    ...
    '    letterIdx = isRight * mix(mix(2.0, 3.0, isMidRight), 1.0, isTop);',
    '    vec2 luv1 = vec2(gridUV.x * 1.5, gridUV.y);',
    '    vec2 luv2 = vec2((gridUV.x - 2.0/3.0) * 3.0, (gridUV.y - ROW_T1) / (1.0 - ROW_T1));',
    ...
    '    letterIdx = isRight * mix(mix(4.0, mix(2.0, 3.0, isMidRight), isAboveThird), 1.0, isTopThird);',
    '    vec2 luv1 = vec2(gridUV.x * 1.5, gridUV.y);',
    ...
    '    float isBottomRight = isRight * (1.0 - isTop);',
    '    letterIdx = isRight * mix(mix(mix(4.0, 5.0, isMidRight), mix(2.0, 3.0, isMidRight), isMidBottom), 1.0, isTop);',
    '    vec2 luv1 = vec2(gridUV.x * 1.5, gridUV.y);',
    '    vec2 luv2 = vec2((gridUV.x - 2.0/3.0) * 3.0, (gridUV.y - ROW_T1B) / (1.0 - ROW_T1B));',
    '    vec2 luv3 = vec2((gridUV.x - 2.0/3.0) * 6.0, (gridUV.y - ROW_T2) / (ROW_T1B - ROW_T2));',
    '    vec2 luv4 = vec2((gridUV.x - 5.0/6.0) * 6.0, (gridUV.y - ROW_T2) / (ROW_T1B - ROW_T2));',
    ...
    '    letterIdx = isRight * mix(mix(mix(4.0, mix(5.0, 6.0, isR3Ge3), isR3Ge2), mix(2.0, 3.0, isMidRight), isAboveBt), 1.0, isRow1);',
    '    vec2 luv1 = vec2(gridUV.x * 1.5, gridUV.y);',
    '    vec2 luv2 = vec2((gridUV.x - 2.0/3.0) * 3.0, (gridUV.y - ROW_T1B) / (1.0 - ROW_T1B));',
    '    vec2 luv3 = vec2((gridUV.x - 2.0/3.0) * 6.0, (gridUV.y - ROW_T2) / (ROW_T1B - ROW_T2));',
    '    vec2 luv4 = vec2((gridUV.x - 5.0/6.0) * 6.0, (gridUV.y - ROW_T2) / (ROW_T1B - ROW_T2));',
    ...
    '    letterIdx = isRight * mix(mix(mix(7.0, mix(4.0, mix(5.0, 6.0, isR3Ge3), isR3Ge2), isAboveR4), mix(2.0, 3.0, isMidRight), isAboveR3), 1.0, isRow1);',
    '    vec2 row3UV = mix(mix(luv5, luv6, isR3Ge2), luv7, isR3Ge3);',
    '    vec4 row3S  = mix(mix(texture(u_tex5, luv5), texture(u_tex6, luv6), isR3Ge2), texture(u_tex7, luv7), isR3Ge3);',
    '    vec2 row2UV = mix(luv3, luv4, isMidRight);',
    '    vec4 row2S  = mix(texture(u_tex3, luv3), texture(u_tex4, luv4), isMidRight);',
    ...
    '    letterIdx = isRight * mix(mix(mix(mix(7.0, 8.0, isMidRight), mix(4.0, mix(5.0, 6.0, isR3Ge3), isR3Ge2), isAboveR4), mix(2.0, 3.0, isMidRight), isAboveR3), 1.0, isRow1);',
    '    vec2 row3UV = mix(mix(luv5, luv6, isR3Ge2), luv7, isR3Ge3);',
    '    vec4 row3S  = mix(mix(texture(u_tex5, luv5), texture(u_tex6, luv6), isR3Ge2), texture(u_tex7, luv7), isR3Ge3);',
    '    vec2 row2UV = mix(luv3, luv4, isMidRight);',
    '    vec4 row2S  = mix(texture(u_tex3, luv3), texture(u_tex4, luv4), isMidRight);',
    ...
    '    letterIdx = isRight * mix(mix(mix(mix(7.0, mix(8.0, 9.0, isR3Ge3), isR3Ge2), mix(4.0, mix(5.0, 6.0, isR3Ge3), isR3Ge2), isAboveR4), mix(2.0, 3.0, isMidRight), isAboveR3), 1.0, isRow1);',
    '    vec2 row3UV = mix(mix(luv5, luv6, isR3Ge2), luv7, isR3Ge3);',
    '    vec4 row3S  = mix(mix(texture(u_tex5, luv5), texture(u_tex6, luv6), isR3Ge2), texture(u_tex7, luv7), isR3Ge3);',
    '    vec2 row2UV = mix(luv3, luv4, isMidRight);',
    '    vec4 row2S  = mix(texture(u_tex3, luv3), texture(u_tex4, luv4), isMidRight);',
    ...
    '    float isRow2Reg   = isRight * isAboveR3 * (1.0 - isRow1);',
    '    float row4i = mix(7.0, mix(8.0, mix(9.0, 10.0, isR4Ge4), isR4Ge3), isR4Ge2);',
    '    letterIdx = isRight * mix(mix(mix(row4i, mix(4.0, mix(5.0, 6.0, isR3Ge3), isR3Ge2), isAboveR4), mix(2.0, 3.0, isMidRight), isAboveR3), 1.0, isRow1);',
    '    float isRow3Reg   = isRight * isAboveR4 * (1.0 - isAboveR3);',
    '    float isRow4Reg   = isRight * (1.0 - isAboveR4);',
    '    vec2 luv1  = vec2(gridUV.x * 1.5, gridUV.y);',
    '    vec2 luv2  = vec2((gridUV.x - 2.0/3.0)   * 3.0,  (gridUV.y - ROW_4A) / (1.0 - ROW_4A));',
    ...
    '  // letterIdx is this letter\'s index, already resolved above for every',
    '  // letter-count layout. It is constant across a letter\'s whole cell and',
    '  // the size is a table lookup keyed off it, so a cohort boundary cannot',
    '  // fall inside one letter and paint half of it — the failure the line and',
    '  // dot shaders each had to be fixed for.',
    '  vec2  el       = paletteElement(letterIdx);',
    ...
    '  // Applied to the letter\'s ink only, before the outline is mixed in — an',
    '  // outline stays its own colour, which is black, which is the shirt.',
    '  fillCol = applyPaletteGroups(fillCol, el.x, letterIdx, el.y);',
    ...
    '  // In invert mode, suppress letterAlpha at border/outer-border pixels so they are transparent',
    '  float activeLetterAlpha = mix(letterAlpha, letterAlpha * (1.0 - max(isBorder, outerB)), u_invert);',
    '  vec3  finalColor = mix(mix(vec3(0.0), letterColor, activeLetterAlpha), u_border_color, activeBorder);',
    '  finalColor = mix(finalColor, u_border_color, activeOuterB);',
    '  float alpha = max(max(activeBorder, activeOuterB), activeLetterAlpha);',
    '',
    '  // ── Corner rounding ───────────────────────────────────────────────────────',
    '  float _du = abs(dFdx(localUV.x));',
    '  float _dv = abs(dFdy(localUV.y));',
    '  float _ca = (_du > 1e-6) ? _dv / _du : 1.0;',
    '  float _r  = min(u_corner_radius, min(0.5 * _ca, 0.5));',
    '  vec2  _p  = (localUV - 0.5) * vec2(_ca, 1.0);',
    '  vec2  _q  = abs(_p) - vec2(0.5 * _ca - _r, 0.5 - _r);',
    '  float _d  = length(max(_q, 0.0)) + min(max(_q.x, _q.y), 0.0) - _r;',
    '  float _aa = abs(dFdy(gridUV.y));',
  ...
  window.ShaderBase.create({
    animateValues:  true,
    instantKeys:    ['u_opacity', 'u_distress_0', 'u_distress_scale_0', 'u_distress_1', 'u_distress_scale_1', 'u_distress_2', 'u_distress_scale_2', 'u_distress_3', 'u_distress_scale_3', 'u_grain_mode', 'u_halftone_shape', 'u_distress_falloff', 'u_grid_aspect', 'u_invert', 'u_vignette_top', 'u_vignette_bottom', 'u_vignette_left', 'u_vignette_right', 'u_vignette_anchor_x', 'u_vignette_anchor_y'],
    fragSrc: fragSrc,
