(function () {
  ...
  window.ShaderBase = {
    ...
    commonGLSL: [
      ...
      '}',
      'float halftoneNoise(float scale) {',
      ...
      '}',
      'float applyDistress(float alpha, vec2 dUV, float distress, float scale, float grainMode, float falloff, float luma) {',
      ...
      '  } else if (grainMode < 3.5) {',
      '    dn = crosshatchNoise(dUV, scale);',
      '  } else {',
      '    dn = halftoneNoise(scale);',
      '  }',
      '  float edgeFactor = mix(1.0, dist, falloff);',
      '  float threshold = grainMode >= 3.5 ? distress * luma * edgeFactor : distress * edgeFactor;',
