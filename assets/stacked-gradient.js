(function () {
  ...
  var fragSrc = [
    ...
    '  vec3 lch3 = oklab_to_oklch(linear_rgb_to_oklab(srgb_to_linear(u_color3)));',
    ...
    '  float inBoundsY = step(0.0, y) * (1.0 - step(1.0, y));',
    '  float baseAlpha = fadeLeft * fadeRight * inBoundsY;',
    '',
    '  // --- Text overlay ---',
    ...
    '  vec3 textFillColor = mix(baseTextColor, srgb_to_linear(u_text_color), u_use_text_color);',
    '',
    '  vec4 base        = vec4(col, baseAlpha);',
    '  vec4 withOutline = mix(base, vec4(srgb_to_linear(u_outline_color), 1.0), outlineSample);',
    ...
    '    // supersample) so dots shrink toward row/fade edges instead of slicing.',
    ...
    '      }',
    '    }',
    '    vec3 dotColor  = covSum > 0.001 ? inkSum / covSum : col;',
    ...
    '    finalAlpha = halftoneNoise(u_distress_scale, halftoneDrive(coverage, dotLuma, cellVig, u_distress)) * u_opacity;',
    '    col = dotColor;',
    '  } else {',
    '    finalAlpha = applyDistress(px.a, dUV, u_distress, u_distress_scale, u_grain_mode, u_distress_falloff, dot(col, vec3(0.299, 0.587, 0.114)), vigMask) * u_opacity;',
    '  }',
    '  // Linear -> sRGB to match Three.js renderer output',
    '  vec3 encoded = pow(col, vec3(1.0 / 2.2));',
