...
// circle-on-line and line-circle share their geometry and their entire JS
// surface (setup/render/textKey/drawText); the assets differ only in the
// vignette style and the cohort-labels debug block. One suite runs against
// both sources so the shared contract cannot silently fork — the two files
// used to carry byte-identical copies of these tests.
const SHADER_NAMES = ['circle-on-line', 'line-circle'];
  ...
  describe(`${name}.js`, () => {
    ...
    it('fragSrc declares all expected uniforms', () => {
      ...
      [
        ...
        expect(frag, `missing uniform ${uniform}`).toContain(uniform);
      });
    });
    ...
    it('setup() returns an object with all required uniform keys', () => {
      const gl = { getUniformLocation: vi.fn((_p, uniform) => ({ _loc: uniform })) };
